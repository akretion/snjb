...
class EasyPaymentWiz(models.TransientModel):
    ...
    payment_method_line_id = fields.Many2one(comodel_name="account.payment.method.line", string="Payment", required=True)
    ...
    def _compute_payment_domain(self):
        for rec in self:
            rec.payment_domain = [("id", "in", self.company_id.quick_payment_method_ids.ids)]
